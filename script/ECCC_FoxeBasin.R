# Source dataset ECCC Foxe Basin 2002
# Author: "Melina Houle"
# Date: "May 11, 2026"
## Note on translation:
#         - Time is recorded in seconds. 
#         - Time is unknown in 2003
#         - Delete 1 obs with no coord
#         - Delete 294 rows with no obs
#----------------------------------------------
#update.packages()
...
if (length(list.files(project_dir)) ==0) {
  ...
  survey <- gd.list %>%
    ...
    select(id)
  drive_download(as_id(as.character(survey)), path = file.path(project_dir, "Foxe_Basin_Data.xlsx"))
  
}
  
survey_tbl <- read_excel(file.path(project_dir, "Foxe_Basin_Data.xlsx"), sheet = "SpeciesCountbyPOLYGON")
